import { createMachine } from 'xstate';

export type EventDone = { type: 'done.invoke.inProgressInvokeUpload' };
export type EventFailed = { type: 'error.platform.inProgressInvokeUpload'; data: { message: string } };
export type EventRetryRequested = { type: 'RETRY_REQUESTED' };
type Event = EventDone | EventFailed | EventRetryRequested;

type TypeState =
  | {
      value: 'done';
      context: string;
    }
  | {
      value: 'inProgress';
      context: string;
    }
  | {
      value: 'failed';
      context: string;
    };

type Ctx = string;
export const uploadMachine =
  /** @xstate-layout  */
  createMachine<Ctx, Event, TypeState>({
    context: 'initial path',
    predictableActionArguments: true,
    // there's a problem with service definition when using typegen in this machine  ¯\_(ツ)_/¯
    initial: 'inProgress',
    states: {
      inProgress: {
        invoke: {
          id: 'inProgressInvokeUpload',
          src: 'uploadFile',
          onDone: 'done',
          onError: 'failed',
        },
      },
      failed: {
        on: {
          RETRY_REQUESTED: 'inProgress',
        },
      },
      done: {
        type: 'final',
      },
    },
  });
